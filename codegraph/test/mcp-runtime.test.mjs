...
test('combinatorial watcher policy: unlink accepts only a previously registered source while ignored and non-source paths stay excluded', async () => {
  ...
  await runtime.start()
  runtime.watcher.emit('unlink', path.join(root, 'src', 'known.ts'))
  runtime.watcher.emit('unlink', path.join(root, 'src', 'notes.txt'))
  runtime.watcher.emit('unlink', path.join(root, 'node_modules', 'ignored.ts'))
...
test('domain analysis: watcher add then unlink for a previously unregistered source coalesces to an absent final state', async () => {
  ...
  runtime.watcher.emit('add', path.join(root, 'src', 'new.ts'))
  runtime.watcher.emit('unlink', path.join(root, 'src', 'new.ts'))
  await runtime.queue.idle()
  await runtime.close()
